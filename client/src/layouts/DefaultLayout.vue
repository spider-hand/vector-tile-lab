<template>
    ...
    <Card class="p-2">
      <div class="flex flex-col gap-1">
        <Button v-for="item in items" :key="item.title" :variant="activeItem === item.title ? 'secondary' : 'ghost'"
          class="justify-start gap-2" @click="handleMenuClick(item.title)">
  ...
  <LayerSidebar v-show="layerSidebarOpen" @close="activeItem = null" />
  <AttributeSidebar v-show="attributeSidebarOpen" @close="activeItem = null" />
</template>
...
import { FileText, SlidersHorizontal, Upload, Tags, Layers } from 'lucide-vue-next'
